...
CARDS = [
    ...
    # ---- R2.2.9 rate equation (AHL) ----
    ("The rate equation is rate = {{c1::k [A]ⁿ [B]ᵐ}}.",
     "k = rate constant · n,m = orders · overall = n+m (R2.2.9)"),
    ("The rate constant k is independent of {{c1::concentration}} but changes with {{c2::temperature}} and {{c2::catalyst}}.",
     "The bouncer's mood (R2.2.11)"),
    ("For rate = k[A]², the units of k are {{c1::mol⁻¹ dm³ s⁻¹}}.",
     "Work out: mol dm⁻³ s⁻¹ ÷ (mol dm⁻³)² (R2.2.11)"),
    ("For a first-order reaction (rate = k[A]), the units of k are {{c1::s⁻¹}}.",
     "mol dm⁻³ s⁻¹ ÷ mol dm⁻³ (R2.2.11)"),
    ("For a zero-order reaction (rate = k), the units of k are {{c1::mol dm⁻³ s⁻¹}}.",
     "Same units as rate itself (R2.2.11)"),
    ("For a third-order reaction, the units of k are {{c1::mol⁻² dm⁶ s⁻¹}}.",
     "Pattern for order n: mol¹⁻ⁿ dm³ⁿ⁻³ s⁻¹ (R2.2.11)"),
    # ---- R2.2.10 orders (AHL) ----
    ("Reaction orders are determined {{c1::experimentally}} — never from the {{c2::balanced equation}}.",
     "THE biggest kinetics trap (R2.2.9/10)"),
    ("If doubling [A] leaves the rate unchanged, the order in A is {{c1::zero}}.",
     "[A]⁰ = 1 — doesn't appear in the rate equation (R2.2.10)"),
    ...
    ("The {{c1::initial rates}} method finds orders by changing ONE concentration while holding the others fixed.",
     "Isolate each reactant's power one at a time (R2.2.10)"),
    ("A straight-line plot of {{c1::ln[A]}} against time confirms the reaction is {{c2::first}} order in A (slope −k).",
     "Linearise to identify the order (R2.2.10)"),
    ("A straight-line plot of {{c1::1/[A]}} against time confirms the reaction is {{c2::second}} order in A (slope +k).",
     "Mg + HCl worked example: 1/[HCl] vs t is linear → 2nd order (R2.2.10)"),
    # ---- R2.2.8 molecularity (SL!) ----
    ("Molecularity is the number of {{c1::reacting particles}} taking part in a single {{c2::elementary step}}.",
     "About ONE step of the mechanism — not the whole reaction (R2.2.8)"),
    ("A step with ONE reacting particle is {{c1::unimolecular}}; two particles → {{c2::bimolecular}}; three → {{c3::termolecular}}.",
     "N₂O₄ → 2NO₂ is unimolecular · NO₂ + CO → NO + CO₂ is bimolecular (R2.2.8)"),
    ("Molecularity is a property of an {{c1::elementary step}}; order is measured from {{c2::experimental data}} for the whole reaction.",
     "They often match for the RDS but are NOT the same thing (R2.2.8)"),
    # ---- R2.2.6 / 2.2.10 rate-determining step ----
    ("The rate-determining step is the {{c1::slowest step}} of a multi-step mechanism.",
     "The club moves as fast as its slowest bouncer (R2.2.6)"),
    ...
    ("Worked example: N₂O₅ decomposition gave gradient −12 500 K → Eₐ = {{c1::104 kJ mol⁻¹}} and A = {{c2::2.85 × 10¹³ s⁻¹}}.",
     "12 500 × 8.31 = 103 875 J mol⁻¹ ≈ 104 kJ (R2.2.12/13)"),
]
